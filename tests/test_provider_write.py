...
from prosekernel.engine import render_provider_write_report, run_provider_write
from prosekernel.providers import ProviderAdapter
...
ProseKernel gives the agent a concrete sequence before it writes: retrieve strong examples, apply named patterns, draft from proof, then score the result for specificity and slop.
...
def test_run_provider_write_uses_injected_adapter_without_api_call():
    ...
    assert "ProseKernel gives the agent" in result.draft
...
def test_render_provider_write_report_includes_trace_and_quality_results():
    ...
    assert "# ProseKernel Write Report" in report
...
def test_cli_write_refuses_to_choose_default_provider(capsys):
    ...
    assert "prosekernel brief" in captured.err
